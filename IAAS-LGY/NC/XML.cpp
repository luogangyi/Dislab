...
string XML::getKvmHeaderXML(string vmName, int ram, int vcpu, string uuid)
{
	...
	xml += "<os><type arch='x86_64' machine='rhel6.4.0'>hvm</type></os><boot dev='hd' />";
	xml += "<features><acpi/><apic/><pae/></features>";

	memset(buf,0,sizeof(buf));
	sprintf(buf,"%d",vcpu);
	xml += "<vcpu  placement='static'>" + string(buf);
	xml += "</vcpu>";
	xml += "<clock offset='utc'/><on_poweroff>destroy</on_poweroff><on_reboot>restart</on_reboot><on_crash>restart</on_crash>";
	xml += "<devices>";
...
string XML::getKvmTailXML()
{
	...
	xml += "</video>";
	xml += "<memballoon model='virtio'><address type='pci' domain='0x0000' bus='0x00' slot='0x06' function='0x0'/></memballoon>";
