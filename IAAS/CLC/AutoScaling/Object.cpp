...
bool Object::sendData2CLC(const char * data, int len, int sock)
{
	String logContent;
	...
	int msgLen = strlen(lenBuff);

	if( send(sock,lenBuff, 4, 0) != 4)
	{

		log.write("Object::sendData2CLC::send message size error.", Log::ERROR);
...
bool Object::recvDataFromCLC(int sock, String& data)
{
	int msgLen = 0;
	String str;
	int recvLen = 0;
...
int Object::connect2Server(const char* ip, int port)
{
	...
	if ( iRet != 0 )
	{
		perror("Object::connect2Server() connect to nc vm server error");
		close(sock);
		return -1;
	}
	return sock;
...
bool Object::checkRight( String name )
{
	//cout<<"authData:"<<authData<<endl;
	if( authData == "-1|-1" )
		return true;
	return authData.hasStr( name );
}


bool Object::sendData2NC(const char * data, int len, int sock)
{
	String logContent;
	...
	if( send(sock,&len, sizeof(len), 0) != sizeof(len))
	{
		...
		log.write("Object::sendData2NC::send message size error.", Log::ERROR);
	...
	if( iSend < 0 )
	{
		log.write("Object::sendData2NC call send error.", Log::ERROR);
		return false;
	}
	int iRet = 0;
	while( iSend < len )
	{
		...
		{
			log.write("Object::sendData2NC call send error in while.", Log::ERROR);
			return false;
		}
	}
...
bool Object::recvDataFromNC(int sock, String& data)
{
	int msgLen = 0;
	String str;
	int recvLen = 0;
	...
	if( iRecv <= 0 )
	{
		log.write("Object::recvDataFromNC receive ack message size error.", Log::ERROR);		
		return false;
	}
	if( iRecv != sizeof(msgLen) )
	{
	...
	while(recvLen<msgLen)
	{
		...
		{
			str = strerror(errno);
			log.write("Object::recvDataFromNC receive ack message error:" + str + ".", Log::ERROR);
