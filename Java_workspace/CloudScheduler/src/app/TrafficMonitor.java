...
public class TrafficMonitor {
	 ...
	 * @param result
	 */
	public void storeResultToDB(int VMID,String result)
	{
		String sql="delete from TrafficData where VMID = '"+VMID+"'";
		String sql2 = "insert into TrafficData (VMID,Data) values ('"+VMID+"','"+result+"')";
		try{
            // statement用来执行SQL语句
            Statement statement = conn.createStatement();
	 ...
	 * @return
	 */
	void sortByGroupInnerIdAsc(List<VMDetail> list)
	{
		Collections.sort(list, 
				new Comparator<VMDetail>(){
			public int compare(VMDetail vm1,VMDetail vm2)
			{
				return vm1.getGroupInnerID()-vm2.getGroupInnerID();
			}
		});
	}
	/**
	 ...
	 */
	public void processData(String fileName,int local_vm_id,String m_ip, List<VMDetail> vm_list)
	{

		Long[] data_arr = new Long[vm_list.size()]; 
		Pattern[] pattern_arr = new Pattern[vm_list.size()];
		//首先必须对vm_list按GroupInnerId升序排列！！
		
		//sort!!!
		sortByGroupInnerIdAsc(vm_list);
		
		String regex_ip = m_ip.replaceAll("\\.", "\\\\.");
		String[] regex_ip_arr = new String[vm_list.size()];
		for(int i=0;i < vm_list.size();i++)
		{
			regex_ip_arr[i] = vm_list.get(i).getMapip().getIP().replaceAll("\\.", "\\\\.");
		...
		//注意！虽然key是VMID，但是是按GroupInnerID排序的！
		String result_data="";
		for(int i=0;i<vm_list.size();i++)
		{
			result_data+=vm_list.get(i).getID();
			result_data+=":";
		...
		//将结果存入数据库中
		storeResultToDB(local_vm_id,result_data);
	}
	
	public List<VMDetail> getGruopPartnerVMList(int vmid)
	{		
		String sql="select GroupID from VMDetail where ID = "+vmid + "";
		List<Integer> id_list = new LinkedList<Integer>();
		...
		try{
    		...
    		sql="select VMDetail.ID, GroupID,GroupInnerID,IP from VMDetail,MACIP where VMDetail.ID = MACIP.VMID and GroupID = "+group_id ;
            // statement用来执行SQL语句
            statement = conn.createStatement();
            rs = statement.executeQuery(sql);
            String name = null;
            ...
            while(rs.next()) {
            	...
            	macip.setIP(rs.getString("IP"));
            	vm.setMapip(macip);
            	vm.setID(rs.getInt("VMDetail.ID"));
            	vm.setGroupID(rs.getInt("GroupID"));
            	vm.setGroupInnerID(rs.getInt("GroupInnerID"));
                //选择sname这列数据
            	vm_list.add(vm);
             System.out.println(group_id+" has parter VM(id) :"+vm.getID()+"GroupInnerID: "+vm.getGroupInnerID()+" IP: "+vm.getMapip().getIP());
            }
            return vm_list;
	...
	public void startMonitor()
	{
		while(true)
		{
			...
			for(int vm_id:local_vm_id_list)
			{
				...
				List<VMDetail> vm_partner_list = getGruopPartnerVMList(vm_id);
				//替换为正则表达式所需要的格式
...
//				processData(monitored_filename,local_vm_ip,parter_ip_list);
				processData(monitored_filename,vm_id,local_vm_ip,vm_partner_list);
